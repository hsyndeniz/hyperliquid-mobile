...
- [src/app/(market)/_layout.tsx](<src/app/(market)/_layout.tsx>) — the market flow: `market/[coin]`, `order`, `pick-market`, a native stack presented as the PLATFORM's sheet by the root layout. Opening a market, switching market and ordering are one errand, so they are one surface; dismissing the sheet ends all of it. UIKit owns the slide-up, the dimmed backdrop, the rounded corners, and a drag-to-dismiss that negotiates with scrolled content natively — drag at scroll-top dismisses, anywhere else scrolls — which is why the card's pages use PLAIN scroll views again. The group keeps the historic hrefs (`/market/BTC`, `/order`, `/pick-market`; a group segment never appears in the path), and one href works from both sides of the sheet boundary: pushed from outside it opens the sheet at that page, from inside it moves within the stack. The `market/[coin]` page is the sheet's only chrome-less page and carries its own `pt-4` so the coin badge is not flush against the rounded corner; `order` keeps the real native back button with no bar (`headerTransparent`, empty title).
...
- The order screen turns the sheet's dismissal off while a submit is in flight, by setting `gestureEnabled` on the PARENT route (`navigation.getParent()` — the `(market)` screen of the root stack). That is not a correctness guard — `placeOrders` journals before it sends, so a dismissed submit still reconciles — it is there because swiping a half-placed order off screen is a horrible thing to do to someone.
